...
func (s *Server) AcceptBlock(ctx context.Context, req *pb.BlockRequest) (*emptypb.Empty, error) {
	...
	for i, tx := range blk.Txs {
		...
		for j, action := range tx.Actions {
			...
			if actionType == vmconsts.CreateAssetID { // create_asset action
    ...
    `, assetID, assetTypeID, assetType, sponsor, txID,
			...
			} else if actionType == vmconsts.RegisterValidatorStakeID {
            ...
            SET stake_end_block = EXCLUDED.stake_end_block,
				...
				if err != nil {
					log.Printf("Error saving validator stake to database: %v\n", err)
				}
			}
		}
